...
export const tasksToPlans = (tasks: Task[], agents: Agent[]): PlanData[] => {
  ...
  return tasks.map(task => {
    ...
    const status = task.status === 'complete' ? 'done' : task.status === 'running' ? 'running' : task.status === 'stopped' ? 'stopped' : 'pending';
    const statusTextVal = status === 'done' ? 'เสร็จสิ้น' : status === 'stopped' ? 'ยกเลิก' : status === 'running' ? 'กำลังทำงาน' : 'รออนุมัติ';
